...
int main() {
    ...
    while (true) {
        ...
        // ── Comandos del monitoreo (REQ/REP) ────────────
        if (items[1].revents & ZMQ_POLLIN) {
            ...
            if (tipo == "AMBULANCIA") {
                string interseccion = extraerValorString(consulta, "interseccion");
                string cmd = "{\"comando\":\"CAMBIAR_LUZ\","
                             "\"interseccion\":\"" + interseccion + "\","
                             "\"estado\":\"VERDE\","
                             "\"duracion_seg\":120,"
                ...
                zmq::message_t cmdMsg(cmd.begin(), cmd.end());
                pushSemaforos.send(cmdMsg, zmq::send_flags::none);
                respuesta = "{\"ok\":true,\"accion\":\"VERDE_FORZADO\",\"interseccion\":\"" + interseccion + "\"}";
                cout << "[ANALITICA] Ambulancia priorizada en " << interseccion << endl;
